// Quiz content for the Claude Certified Architect — Foundations exam.
//
// Source of truth: docs/sources/quizData.js (verbatim content), retyped here.
// Mutation forbidden — this object is part of the "living study guide" and is
// referenced by the Domain pages and pattern showcase.
...
export interface QuizQuestion {
  ...
  explanation: string;
  wrongExplanations?: Partial<Record<OptionLetter, string>>;
  studyArea?: string;
  studyAreaLink?: string;
}
    ...
    questions: [
      {
        ...
        options: [
          ...
          { letter: 'B', text: 'Use synchronous calls for PR style checks and nightly test generation; use Message Batches API only for weekly security audits.' },
          { letter: 'C', text: 'Use synchronous calls for all three tasks for consistent response times, and rely on prompt caching to reduce costs across all workloads.' },
          { letter: 'D', text: 'Use the Message Batches API for all three tasks to maximize the 50% cost savings, and configure the pipeline to poll for batch completion.' },
        ],
        correct: 'A',
        explanation:
        ...
        studyArea: 'Claude Code for Continuous Integration — review Batch Processing concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Add few-shot examples of misleading comments to help the model recognize similar patterns in the codebase' },
          { letter: 'B', text: 'Specify explicit criteria: flag comments only when their claimed behavior contradicts actual code behavior' },
          { letter: 'C', text: 'Include git blame data so Claude can identify comments that predate recent code modifications' },
          { letter: 'D', text: 'Filter out TODO, FIXME, and descriptive comment patterns before analysis to reduce noise' },
        ],
        correct: 'B',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Keep all categories enabled while adding few-shot examples to improve each category\'s accuracy over the coming weeks.' },
          { letter: 'B', text: 'Apply a uniform strictness reduction across all categories to bring the overall false positive rate to an acceptable level.' },
          { letter: 'C', text: 'Keep all categories but display a confidence score with each finding, letting developers decide which to investigate.' },
          { letter: 'D', text: 'Temporarily disable high false positive categories (style, naming, documentation) and run only high-precision categories while improving prompts.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'The batch API doesn\'t support tool definitions in request parameters.' },
          { letter: 'B', text: 'The asynchronous model prevents executing tools mid-request and returning results for Claude to continue analysis.' },
          { letter: 'C', text: 'Batch processing lacks request correlation identifiers for matching outputs to input requests.' },
          { letter: 'D', text: 'Batch processing latency of up to 24 hours is too slow for pull request feedback, though the workflow could otherwise function.' },
        ],
        correct: 'B',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Include explicit formatting instructions in your review prompt requiring each finding to follow a parseable template like [FILE:path] [LINE:n] [SEVERITY:level] ....' },
          { letter: 'B', text: 'Keep the narrative review format but add a summarization step that uses Claude to generate a structured JSON summary of the findings.' },
          { letter: 'C', text: 'Add a "Review Output Format" section to CLAUDE.md with examples showing structured findings, so Claude learns the expected format from project context.' },
          { letter: 'D', text: 'Use CLI flags --output-format json and --json-schema to enforce structured findings, then parse output to post inline comments via the GitHub API.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Implement post-processing that filters suggestions whose descriptions match keywords from existing test names' },
          { letter: 'B', text: 'Add instructions directing Claude to focus exclusively on edge cases and error conditions rather than successful paths' },
          { letter: 'C', text: 'Reduce requested suggestions from 10 to 5, assuming Claude will prioritize the most valuable cases first' },
          { letter: 'D', text: 'Include the existing test file in the context so Claude can identify what scenarios are already covered' },
        ],
        correct: 'D',
        explanation:
        ...
        studyArea: 'Claude Code for Continuous Integration — review Context Provision Methods concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Add 3-4 few-shot examples showing the exact format you want: issue identified, code location, specific fix suggestion' },
          { letter: 'B', text: 'Implement a two-pass approach where one prompt identifies issues and a second prompt generates fixes, allowing specialization' },
          { letter: 'C', text: 'Further refine the instructions with more explicit requirements for each part of the feedback format (location, issue, severity, suggested fix)' },
          { letter: 'D', text: 'Expand the context window to include more of the surrounding codebase so the model has sufficient information to suggest specific fixes' },
        ],
        correct: 'A',
        explanation:
        ...
        studyArea: 'Claude Code for Continuous Integration — review Few-Shot Prompting concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Have a second, independent Claude Code instance review the changes without seeing the generator\'s reasoning.' },
          { letter: 'B', text: 'Include comprehensive test files and documentation in the prompt context so Claude better understands expected behavior during generation.' },
          { letter: 'C', text: 'Enable extended thinking mode for the generation pass, allowing more thorough deliberation before producing suggestions.' },
          { letter: 'D', text: 'Add explicit self-review instructions to the generation prompt, asking Claude to critique its own suggestions before finalizing output.' },
        ],
        correct: 'A',
        explanation:
        ...
        studyArea: 'Claude Code for Continuous Integration — review Multi-Instance Verification concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Require Claude to include its reasoning and confidence assessment inline with each finding' },
          { letter: 'B', text: 'Add a post-processor that analyzes finding patterns and automatically suppresses those matching historical false positive signatures' },
          { letter: 'C', text: 'Configure Claude to only surface findings it assesses as high confidence, filtering out uncertain flags before developers see them' },
          { letter: 'D', text: 'Categorize findings as "blocking issues" versus "suggestions" with tiered review requirements' },
        ],
        correct: 'A',
        explanation:
        ...
        studyArea: 'Claude Code for Continuous Integration — review False Positive Reduction concepts in the exam study guide.',
        studyAreaLink: '/domains/d4#task-4.2',
      },
      {
        id: 12,
        ...
        options: [
          { letter: 'A', text: 'Request that Claude include its reasoning for each severity assignment, then use that reasoning to manually calibrate and adjust ratings during review' },
          { letter: 'B', text: 'Include explicit severity criteria in your prompt with concrete code examples for each severity level' },
          { letter: 'C', text: 'Modify the prompt to ask Claude to rate severity relative to other issues in the same PR, so the most severe issue is always marked critical and others rated proportionally' },
          { letter: 'D', text: 'Add a CLAUDE.md file that lists issue types and their default severities, instructing Claude to reference this mapping when assigning ratings' },
        ],
        correct: 'B',
        explanation:
        ...
        studyArea: 'Claude Code for Continuous Integration — review Classification Consistency concepts in the exam study guide.',
        studyAreaLink: '/domains/d4#task-4.2',
      },
      {
        id: 13,
        ...
        options: [
          { letter: 'A', text: 'Include prior review findings in context, instructing Claude to only report new or still-unaddressed issues.' },
          { letter: 'B', text: 'Add a post-processing filter that removes findings matching previous file paths and issue descriptions before posting comments.' },
          { letter: 'C', text: 'Restrict the review scope to only files modified in the most recent push, excluding files from earlier commits.' },
          { letter: 'D', text: 'Run reviews only on initial PR creation and final pre-merge state, skipping intermediate commits.' },
        ],
        correct: 'A',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Switch both workflows to batch processing with status polling to check for completion.' },
          { letter: 'B', text: 'Switch both to batch processing with a timeout fallback to real-time if batches take too long.' },
          { letter: 'C', text: 'Keep real-time calls for both workflows to avoid batch result ordering issues.' },
          { letter: 'D', text: 'Use batch processing for the technical debt reports only; keep real-time calls for pre-merge checks.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Split into focused passes: analyze each file individually for local issues, then run a separate integration-focused pass examining cross-file data flow.' },
          { letter: 'B', text: 'Run three independent review passes on the full PR and only flag issues that appear in at least two of the three runs.' },
          { letter: 'C', text: 'Require developers to split large PRs into smaller submissions of 3-4 files before the automated review runs.' },
          { letter: 'D', text: 'Switch to a higher-tier model with a larger context window to give all 14 files adequate attention in one pass.' },
        ],
        correct: 'A',
        explanation:
    ...
    questions: [
      {
        ...
        options: [
          { letter: 'A', text: 'Add few-shot examples covering every possible order-related query pattern to the system prompt' },
          { letter: 'B', text: 'Review tool descriptions to ensure they clearly distinguish each tool\'s purpose' },
          { letter: 'C', text: 'Implement a pre-processing classifier that detects order queries and routes directly to lookup_order' },
          { letter: 'D', text: 'Reduce the number of tools available to the agent to simplify selection' },
        ],
        correct: 'B',
        explanation:
        ...
        studyArea: 'Customer Support Resolution Agent — review Tool Selection Reliability concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Enhance the system prompt to state that customer verification via get_customer is mandatory before any order operations.' },
          { letter: 'B', text: 'Implement a routing classifier that analyzes each request and enables only the subset of tools appropriate for that request type.' },
          { letter: 'C', text: 'Add a programmatic prerequisite that blocks lookup_order and process_refund calls until get_customer has returned a verified customer ID.' },
          { letter: 'D', text: 'Add few-shot examples showing the agent always calling get_customer first, even when customers volunteer order details.' },
        ],
        correct: 'C',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Implement a preprocessing layer that uses a separate model call to decompose multi-concern messages into individual requests, process each independently, then combine the results.' },
          { letter: 'B', text: 'Consolidate related tools into fewer, more general-purpose tools.' },
          { letter: 'C', text: 'Add few-shot examples to your prompt demonstrating the correct reasoning and tool sequence for multi-concern requests.' },
          { letter: 'D', text: 'Implement response validation that detects incomplete responses and automatically re-prompts the agent to address any missed concerns.' },
        ],
        correct: 'C',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Add explicit verification gates between steps requiring the agent to checkpoint after resolving each concern before moving to the next.' },
          { letter: 'B', text: 'Reduce the number of available tools by consolidating get_customer, lookup_order, and billing-related lookups into a single investigate_issue tool.' },
          ...
          { letter: 'D', text: 'Add few-shot examples demonstrating ideal tool call sequences for various multi-part billing scenarios to your system prompt.' },
        ],
        correct: 'C',
        explanation:
        ...
        studyArea: 'Customer Support Resolution Agent — review Multi-step Workflow Orchestration concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Increase max_tokens to give Claude more space to plan ahead and naturally batch its tool requests.' },
          { letter: 'B', text: 'Implement speculative execution that automatically calls likely-needed tools alongside any requested tool, returning all results regardless of what was requested.' },
          { letter: 'C', text: 'Prompt Claude to batch tool requests per turn, and return all tool results together before the next API call.' },
          { letter: 'D', text: 'Create composite tools like get_customer_with_orders that bundle common lookup combinations into single calls.' },
        ],
        correct: 'C',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'The tool descriptions need additional negative examples specifying when NOT to use each tool to prevent this keyword-triggered confusion' },
          { letter: 'B', text: 'The system prompt contains keyword-sensitive instructions that steer behavior based on terms like "account," creating unintended tool selection patterns' },
          { letter: 'C', text: 'The model\'s base training creates associations between "account" terminology and customer-related operations that override the tool descriptions' },
          { letter: 'D', text: 'The model requires more training data on multi-concept messages and should be fine-tuned on examples that include both account and order language' },
        ],
        correct: 'B',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Parse Claude\'s response text for phrases like "I\'ve completed" or "Is there anything else?"—these natural language signals indicate the task is finished.' },
          { letter: 'B', text: 'Set a maximum iteration count (e.g., 10 calls) and stop when reached, regardless of whether Claude indicates more work is needed.' },
          { letter: 'C', text: 'Check the stop_reason field in Claude\'s response—continue when it equals "tool_use" and stop when it equals "end_turn".' },
          { letter: 'D', text: 'Check whether the response includes any assistant text content—if Claude generated explanatory text, the loop should end.' },
        ],
        correct: 'C',
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          { letter: 'B', text: 'Add examples grouped by tool—all get_customer scenarios together, then all lookup_order scenarios.' },
          { letter: 'C', text: 'Add explicit "use when" and "do not use when" guidelines in each tool\'s description covering the ambiguous cases.' },
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Use a PostToolUse hook to intercept tool results and apply formatting transformations before agent processing' },
          { letter: 'B', text: 'Create a normalize_data tool that the agent calls after each data retrieval to transform values' },
          { letter: 'C', text: 'Modify tools you control to return human-readable formats; create wrapper tools for third-party tools' },
          { letter: 'D', text: 'Add detailed format documentation to your system prompt explaining each tool\'s data conventions' },
        ],
        correct: 'A',
        explanation:
        ...
        studyArea: 'Customer Support Resolution Agent — review Agent SDK Hook Patterns concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Store full conversation history externally and implement retrieval to search it when the agent detects reference phrases like "as I mentioned."' },
          { letter: 'B', text: 'Increase the summarization threshold from 70% to 85% capacity so conversations have more room before summarization triggers.' },
          { letter: 'C', text: 'Revise the summarization prompt to explicitly preserve all numerical values, percentages, dates, and customer-stated expectations verbatim.' },
          { letter: 'D', text: 'Extract transactional facts (amounts, dates, order numbers) into a persistent "case facts" block included in each prompt, outside the summarized history.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Add few-shot examples showing Claude how to use conversational context (products mentioned, dates referenced) to infer the correct customer without requiring clarification.' },
          { letter: 'B', text: 'Modify get_customer to return only the single most likely match based on a ranking algorithm, simplifying Claude\'s decision by eliminating ambiguous results.' },
          { letter: 'C', text: 'Implement a confidence scoring system that proceeds automatically above 85% confidence and prompts for clarification below that threshold.' },
          { letter: 'D', text: 'Instruct Claude to ask for an additional identifier (email, phone, or order number) when get_customer returns multiple matches, before taking any customer-specific action.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Expand each tool\'s description to include input formats it handles, example queries, edge cases, and boundaries explaining when to use it versus similar tools.' },
          { letter: 'B', text: 'Add few-shot examples to the system prompt demonstrating correct tool selection patterns, with 5-8 examples showing order-related queries routing to lookup_order.' },
          { letter: 'C', text: 'Consolidate both tools into a single lookup_entity tool that accepts any identifier and internally determines which backend to query.' },
          { letter: 'D', text: 'Implement a routing layer that parses user input before each turn and pre-selects the appropriate tool based on detected keywords and identifier patterns.' },
        ],
        correct: 'A',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Increase the model tier from Haiku to Sonnet for complex cases, routing based on detected case complexity.' },
          { letter: 'B', text: 'Add a confirmation step where the agent asks "Does this fully address your concern?" before closing, letting customers request additional information if needed.' },
          { letter: 'C', text: 'Add a self-critique step where the agent evaluates its draft response for completeness—ensuring it addresses the customer\'s concern, includes relevant context, and anticipates follow-up questions.' },
          { letter: 'D', text: 'Implement few-shot examples in the system prompt showing complete resolution explanations for five common complex case types, demonstrating how to include policy context, timelines, and next steps.' },
        ],
        correct: 'C',
        explanation:
        ...
        studyArea: 'Customer Support Resolution Agent — review Self-Evaluation Patterns concepts in the exam study guide.',
        studyAreaLink: '/domains/d4#task-4.6',
      },
      {
        id: 14,
        ...
        options: [
          { letter: 'A', text: 'Have the agent self-report a confidence score (1-10) before each response and automatically route requests to humans when confidence falls below a threshold.' },
          { letter: 'B', text: 'Deploy a separate classifier model trained on historical tickets to predict which requests need escalation before the main agent begins processing.' },
          { letter: 'C', text: 'Add explicit escalation criteria to your system prompt with few-shot examples demonstrating when to escalate versus resolve autonomously.' },
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'The customer claims they never received their order, but tracking shows it was delivered and signed for at their address three days ago. The agent should escalate because presenting contradictory evidence might damage the customer relationship.' },
          { letter: 'B', text: 'The customer\'s message mentions both a billing question and a product return. The agent should escalate so a human can coordinate handling both issues in a single interaction.' },
          { letter: 'C', text: 'The customer requests a price match against a competitor. Your policies allow adjustments for price drops on your own site within 14 days but are silent on competitor pricing. The agent should escalate for policy interpretation.' },
          { letter: 'D', text: 'The customer wants to cancel an order that shipped yesterday, with delivery scheduled for tomorrow. The agent should escalate because the customer might change their mind once they receive the package.' },
        ],
        correct: 'C',
        explanation:
        ...
        studyArea: 'Customer Support Resolution Agent — review Escalation Decisions concepts in the exam study guide.',
    ...
    questions: [
      {
        ...
        options: [
          { letter: 'A', text: 'Use direct execution with comprehensive upfront instructions detailing exactly how each service should be structured.' },
          ...
          { letter: 'C', text: 'Begin in direct execution mode and only switch to plan mode if you encounter unexpected complexity during implementation.' },
          { letter: 'D', text: 'Enter plan mode to explore the codebase, understand dependencies, and design an implementation approach before making changes.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Consolidate all conventions in the root CLAUDE.md file under headers for each area, relying on Claude to infer which section applies' },
          { letter: 'B', text: 'Place a separate CLAUDE.md file in each subdirectory containing that area\'s specific conventions' },
          ...
          { letter: 'D', text: 'Create rule files in .claude/rules/ with YAML frontmatter specifying glob patterns to conditionally apply conventions based on file paths' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Add instructions to the skill to compress all outputs into a brief summary before displaying' },
          { letter: 'B', text: 'Split the skill into three smaller skills that each generate less output' },
          { letter: 'C', text: 'Add context: fork to the skill\'s frontmatter to run the analysis in an isolated sub-agent context' },
          { letter: 'D', text: 'Add model: haiku to the frontmatter to use a faster, more efficient model for the analysis' },
        ],
        correct: 'C',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Create an MCP server wrapper that reads tokens from a .env file and proxies requests to the GitHub API, then add this wrapper to your project .mcp.json.' },
          { letter: 'B', text: 'Configure the server in project scope with a placeholder token value, then instruct developers to override it in their local scope configuration.' },
          { letter: 'C', text: 'Have each developer configure the server in user scope with claude mcp add --scope user.' },
          { letter: 'D', text: 'Add the server to a project-scoped .mcp.json with environment variable expansion (${GITHUB_TOKEN}) for authentication, and document the required environment variable in your project README.' },
        ],
        correct: 'D',
        explanation:
        ...
        studyArea: 'Code Generation with Claude Code — review MCP Server Integration concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Rewrite your requirements with greater technical precision, specifying exact field mappings, nesting rules, and timestamp format strings.' },
          { letter: 'B', text: 'Provide 2-3 concrete input-output examples showing the expected transformation for representative API responses.' },
          { letter: 'C', text: 'Ask Claude to explain its current interpretation of the requirements so you can identify where understanding diverges.' },
          { letter: 'D', text: 'Write a JSON schema defining the expected output structure and validate Claude\'s output against it after each iteration.' },
        ],
        correct: 'B',
        explanation:
        ...
        studyArea: 'Code Generation with Claude Code — review Iterative Refinement concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Claude Code caches CLAUDE.md contents after first read. The original developers have cached versions while the new developer loaded after the file was modified. Have all developers clear their Claude Code cache.' },
          { letter: 'B', text: 'The guideline exists in the original developers\' ~/.claude/CLAUDE.md files (user-level) instead of the project\'s .claude/CLAUDE.md. Move the instruction to the project-level file so all team members receive it.' },
          { letter: 'C', text: 'The new developer\'s ~/.claude/CLAUDE.md contains conflicting instructions that override the project settings. Have them remove the conflicting section from their user-level configuration.' },
          { letter: 'D', text: 'Claude Code builds per-user preference models over time through repeated interactions. The new developer needs to repeatedly specify the error handling requirement until Claude learns their preferences.' },
        ],
        correct: 'B',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Keep all content in CLAUDE.md but use @import syntax to organize it into separately maintained files by category' },
          { letter: 'B', text: 'Move all guidance into separate Skills files organized by workflow type, keeping only a brief project description in CLAUDE.md' },
          { letter: 'C', text: 'Keep universal standards in CLAUDE.md and create Skills for task-specific workflows (PR reviews, deployments, migrations) with trigger keywords' },
          { letter: 'D', text: 'Split the CLAUDE.md into files in .claude/rules/ with path-specific glob patterns so each rule loads only for matching file types' },
        ],
        correct: 'C',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Create a personal version at ~/.claude/skills/commit/SKILL.md with the same name' },
          { letter: 'B', text: 'Add username-based conditional logic to the project skill\'s frontmatter' },
          { letter: 'C', text: 'Create a personal version in ~/.claude/skills/ with a different name like /my-commit' },
          { letter: 'D', text: 'Set override: true in the personal skill\'s frontmatter to take precedence over the project version' },
        ],
        correct: 'C',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Split the skill into two separate skills—/explore-start and /explore-end—to demarcate when exploration context should be discarded.' },
          { letter: 'B', text: 'Use the ! prefix in the skill to execute the exploration logic as a bash subprocess.' },
          { letter: 'C', text: 'Create the skill in ~/.claude/skills/ instead of .claude/skills/.' },
          { letter: 'D', text: 'Add context: fork to the skill\'s frontmatter.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Start direct execution to scaffold the Slack channel class following existing patterns, deferring the integration method decision until later.' },
          { letter: 'B', text: 'Start direct execution using incoming webhooks to match the existing one-way notification pattern.' },
          { letter: 'C', text: 'Start direct execution using the bot token approach to enable delivery confirmation capabilities.' },
          { letter: 'D', text: 'Enter plan mode to explore the integration options and their architectural implications, then present a recommendation before implementing.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Create a skill that references the exemplar endpoints and includes pattern-following instructions, invoked on-demand via slash command.' },
          { letter: 'B', text: 'Configure path-specific rules in .claude/rules/api/ that include the exemplar code and activate when working in the API directory.' },
          { letter: 'C', text: 'Reference the exemplar endpoints manually in each generation request by copying relevant code into your prompt.' },
          { letter: 'D', text: 'Add the exemplar endpoint code with pattern documentation to the project CLAUDE.md file so it\'s automatically available.' },
        ],
        correct: 'A',
        explanation:
        ...
        studyArea: 'Code Generation with Claude Code — review Custom Slash Commands concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'In the CLAUDE.md file at the project root' },
          { letter: 'B', text: 'In ~/.claude/commands/ in each developer\'s home directory' },
          { letter: 'C', text: 'In a .claude/config.json file with a commands array' },
          { letter: 'D', text: 'In the .claude/commands/ directory in the project repository' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Create separate markdown files in .claude/rules/, each covering one topic (e.g., testing.md, api-conventions.md)' },
          { letter: 'B', text: 'Create multiple files named CLAUDE.md at different levels of the directory tree, each one overriding the parent\'s instructions' },
          { letter: 'C', text: 'Define a .claude/config.yaml file that maps file patterns to specific sections within CLAUDE.md' },
          { letter: 'D', text: 'Split instructions into README.md files in relevant subdirectories, which Claude automatically loads as instructions' },
        ],
        correct: 'A',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Add argument-hint frontmatter to prompt for required parameters, use context: fork to isolate execution, and restrict allowed-tools to file write operations.' },
          { letter: 'B', text: 'Use positional parameters $1 and $2 instead of $ARGUMENTS to enforce specific inputs, include explicit schema file references via @ syntax to control context, and add description frontmatter warning about destructive operations.' },
          { letter: 'C', text: 'Include validation instructions in the skill\'s SKILL.md that direct Claude to verify $ARGUMENTS contains a valid name, add prompts to ignore prior conversation context, and list forbidden operations Claude should avoid.' },
          { letter: 'D', text: 'Split into separate /migration-create and /migration-apply skills, add instructions in each SKILL.md to request a migration name if not provided, and use different allowed-tools scopes for each skill.' },
        ],
        correct: 'A',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Switch to headless mode with --continue, passing explicit context summaries between batch invocations to maintain continuity.' },
          { letter: 'B', text: 'Define your error handling pattern in CLAUDE.md, then process files in batches across multiple sessions, relying on the shared memory file for consistency.' },
          { letter: 'C', text: 'Use the Explore subagent for Phase 1 to isolate verbose output and return a summary, then continue Phases 2-3 in the main conversation.' },
          { letter: 'D', text: 'Continue all phases in the main conversation, using /compact periodically to reduce context usage as you progress through the files.' },
        ],
        correct: 'C',
        explanation:
    ...
    questions: [
      {
        ...
        options: [
          { letter: 'A', text: 'Create a dedicated error-handling agent that monitors all subagent failures via a shared queue and makes recovery decisions independently, dispatching retry commands directly to subagents.' },
          { letter: 'B', text: 'Have the coordinator validate all documents before dispatching to the subagent, rejecting documents likely to cause failures to ensure the subagent only receives processable files.' },
          { letter: 'C', text: 'Configure the subagent to always return partial results with success status, embedding error details in metadata. The coordinator treats all responses as successful and filters problematic items during synthesis.' },
          { letter: 'D', text: 'Have the subagent implement local recovery for transient failures and only propagate errors it cannot resolve to the coordinator, including what was attempted and any partial results obtained.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Store findings in a vector database and give the synthesis agent retrieval tools to query during its work' },
          { letter: 'B', text: 'Modify upstream agents to return structured data (key facts, citations, relevance scores) instead of verbose content and reasoning' },
          { letter: 'C', text: 'Have the synthesis agent process findings in sequential batches, maintaining running state between calls' },
          { letter: 'D', text: 'Add an intermediate summarization agent that condenses findings before passing to synthesis' },
        ],
        correct: 'B',
        explanation:
        ...
        studyArea: 'Multi-Agent Research System — review Multi-Agent Orchestration concepts in the exam study guide.',
        studyAreaLink: '/domains/d1#task-1.2',
      },
      {
        id: 3,
        ...
        options: [
          ...
          { letter: 'B', text: 'Have the web search agent proactively cache extra context around each source during initial research, anticipating what the synthesis agent might need to verify.' },
          { letter: 'C', text: 'Have the synthesis agent accumulate all verification needs and return them as a batch to the coordinator at the end of its pass, which then sends them all to the web search agent at once.' },
          { letter: 'D', text: 'Give the synthesis agent access to all web search tools so it can handle any verification need directly without round-trips through the coordinator.' },
        ],
        correct: 'A',
        explanation:
        ...
        studyArea: 'Multi-Agent Research System — review Tool Distribution concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Have the subagent retry transient failures internally and only report persistent errors.' },
          { letter: 'B', text: 'Distinguish access failures (timeout) needing retry decisions from valid empty results ("0 results") representing successful queries.' },
          { letter: 'C', text: 'Aggregate outcomes into a single success rate metric (e.g., "67% source coverage") with detailed logs available on request.' },
          { letter: 'D', text: 'Report both the timeout and "0 results" as failures requiring coordinator intervention.' },
        ],
        correct: 'B',
        explanation:
        ...
        studyArea: 'Multi-Agent Research System — review Error Propagation concepts in the exam study guide.',
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Silently skip the corrupted document and continue processing other files to avoid interrupting the workflow.' },
          { letter: 'B', text: 'Throw an exception that terminates the entire research workflow.' },
          { letter: 'C', text: 'Automatically retry parsing the document three times with exponential backoff before reporting failure.' },
          { letter: 'D', text: 'Return the error with context to the coordinator agent, letting it decide how to proceed.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Replace fetch_url with a load_document tool that validates URLs point to document formats.' },
          { letter: 'B', text: 'Add instructions to the document analysis agent\'s prompt clarifying it should only use fetch_url for loading document URLs, not searching.' },
          { letter: 'C', text: 'Implement filtering that blocks fetch_url calls to known search engine domains while allowing other URLs.' },
          { letter: 'D', text: 'Remove fetch_url from the document analysis agent and route all URL loading through the coordinator to the web search agent.' },
        ],
        correct: 'A',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Propagate the timeout exception directly to a top-level handler that terminates the entire research workflow.' },
          { letter: 'B', text: 'Return structured error context to the coordinator including the failure type, the attempted query, any partial results, and potential alternative approaches.' },
          { letter: 'C', text: 'Implement automatic retry logic with exponential backoff within the subagent, returning a generic "search unavailable" status only after all retries are exhausted.' },
          { letter: 'D', text: 'Catch the timeout within the subagent and return an empty result set marked as successful.' },
        ],
        correct: 'B',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Convert to sequential execution where document analysis runs only after web search completes, using the web search findings as context to avoid duplication' },
          { letter: 'B', text: 'Have the coordinator explicitly partition the research space before delegation, assigning distinct subtopics or source types to each agent' },
          { letter: 'C', text: 'Allow both agents to complete their parallel work, then have the coordinator deduplicate overlapping findings before passing to the synthesis agent' },
          { letter: 'D', text: 'Implement a shared state mechanism where agents log their current focus area, allowing other agents to dynamically avoid duplicating work in progress' },
        ],
        correct: 'B',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Rename the web search tool to extract_web_results and update its description to "processes and returns information retrieved from web searches and URLs."' },
          { letter: 'B', text: 'Add few-shot examples to the coordinator\'s prompt showing correct routing: "User uploads quarterly report → document analysis agent" and "User asks about a webpage → web search agent."' },
          { letter: 'C', text: 'Expand the document analysis tool\'s description to include example use cases like "Use for uploaded PDFs, Word documents, and spreadsheets" while leaving the web search tool unchanged.' },
          { letter: 'D', text: 'Add a pre-routing classifier that determines whether the user is referencing uploaded files or web content before the coordinator makes delegation decisions.' },
        ],
        correct: 'A',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Apply source credibility heuristics to select the most likely accurate figure, complete the analysis using that value, and include a footnote mentioning the discrepancy.' },
          { letter: 'B', text: 'Include both figures in the analysis output without flagging them as conflicting, allowing the synthesis agent to determine which to use based on the broader research context.' },
          { letter: 'C', text: 'Complete the document analysis with both figures included, explicitly annotate the conflict with source attribution, and let the coordinator decide how to reconcile before passing to synthesis.' },
          { letter: 'D', text: 'Halt analysis and escalate to the coordinator immediately, asking it to determine which source is authoritative before the agent continues processing remaining documents.' },
        ],
        correct: 'C',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Stream subagent results to the synthesis agent incrementally, processing web search results first to completion before introducing document analysis findings.' },
          { letter: 'B', text: 'Implement rotation that alternates which subagent\'s results appear first across different research tasks, ensuring both sources receive primacy positioning equally over time.' },
          { letter: 'C', text: 'Summarize all subagent outputs to under 20K tokens total before aggregation, ensuring content stays within the model\'s reliable processing range.' },
          { letter: 'D', text: 'Place a key findings summary at the beginning of the aggregated input and organize detailed results with explicit section headers for easier navigation.' },
        ],
        correct: 'D',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'Have the synthesis subagent return an error to the coordinator indicating incomplete upstream data, triggering a full retry or task failure.' },
          { letter: 'B', text: 'Structure the synthesis output with coverage annotations indicating which findings are well-supported versus which topic areas have gaps due to unavailable sources.' },
          { letter: 'C', text: 'Have the synthesis subagent request the coordinator retry the timed-out sources with extended timeouts before proceeding, ensuring complete data coverage before synthesis begins.' },
          { letter: 'D', text: 'Proceed with synthesis using only the successful sources, generating output without indicating which data was unavailable.' },
        ],
        correct: 'B',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'The coordinator batches multiple subagent requests together, reducing the total number of API calls and overall latency' },
          { letter: 'B', text: 'Routing through the coordinator enables automatic retry logic that direct agent-to-agent calls cannot support' },
          { letter: 'C', text: 'The coordinator can observe all interactions, handle errors consistently, and decide what information each subagent should receive' },
          { letter: 'D', text: 'Subagents operate with isolated memory, and direct communication would require complex serialization that only the coordinator can perform' },
        ],
        correct: 'C',
        explanation:
      ...
      },
      {
        ...
        options: [
          { letter: 'A', text: 'The web search agent\'s queries are not comprehensive enough and need to be expanded to cover more creative industry sectors.' },
          { letter: 'B', text: 'The document analysis agent is filtering out sources related to non-visual creative industries due to overly restrictive relevance criteria.' },
          { letter: 'C', text: 'The coordinator agent\'s task decomposition is too narrow, resulting in subagent assignments that don\'t cover all relevant domains of the topic.' },
          { letter: 'D', text: 'The synthesis agent lacks instructions for identifying coverage gaps in the findings it receives from other agents.' },
        ],
        correct: 'C',
        explanation:
...
export function getQuizSection(id: string): QuizSection | undefined {
  return quizSections.find((s) => s.id === id);
}

export function getQuestion(sectionId: string, questionId: number): QuizQuestion | undefined {
  return getQuizSection(sectionId)?.questions.find((q) => q.id === questionId);
}

export const totalQuestionCount = quizSections.reduce((acc, s) => acc + s.questions.length, 0);
