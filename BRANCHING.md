...
#### Feature Branches: `feature/AAIP-{number}-{description}`

**Examples**:
```
feature/AAIP-001-implement-auth
feature/AAIP-002-add-dashboard
```
...
#### Bugfix Branches: `bugfix/AAIP-{number}-{description}`

**Examples**:
```
bugfix/AAIP-010-fix-login-error
bugfix/AAIP-011-resolve-crash
```
...
git checkout -b feature/AAIP-XXX-description {{DEV_BRANCH}}
```
...
git checkout -b feature/AAIP-042-add-search {{DEV_BRANCH}}
...
git merge feature/AAIP-042-add-search
git branch -d feature/AAIP-042-add-search
```
...
git checkout -b feature/AAIP-XXX {{DEV_BRANCH}}
...
- Reference issues: `Closes AAIP-XXX`
...
3. **Create feature branch**: `git checkout -b feature/AAIP-XXX-description`
...
- Reference ticket numbers (AAIP-XXX)
...
```markdown
I'm going to:
1. Create branch: feature/AAIP-XXX-feature-name
...
### Examples
```
AAIP-001: Implement user authentication
AAIP-002: Add dashboard widgets
AAIP-003: Fix login redirect issue
```
...
```
BRANCH PATTERNS:
feature/AAIP-XXX-description    → New features
bugfix/AAIP-XXX-description     → Bug fixes
