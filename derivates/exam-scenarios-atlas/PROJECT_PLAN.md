...
| **v0.4.0 — Adaptive drill** | The drill starts using the accuracy it already records: a **"Drill weak spots"** run mode that weights item selection toward low-accuracy scenarios/domains (unseen counts as weakest), an **accuracy breakdown** (per scenario S1–S6 and per domain D1–D5) on the drill idle screen, and **per-scenario drill entry** from each scenario page (`/drill?scenario=n` filters to that scenario's items). Persistence shape unchanged (`esa:drill:v1`). | ✅ Shipped. Weighted sampling without replacement (`weight = 1 + 3·(1 − accuracy)`, unseen = 4, clamped against corrupt buckets); stats load through `normalizeStats()` so wrong-shape localStorage degrades instead of crashing; breakdown rows link to focused runs; accuracy chips on scenario pages + home tile; run-complete offers a weak-spot run; "forget my drill history" with two-step confirm; `npm run check:drill` keeps the sampling honest offline. |
