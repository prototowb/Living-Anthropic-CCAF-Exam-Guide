...
| [`exam-scenarios-atlas/`](./exam-scenarios-atlas/PROJECT_PLAN.md) | The **six exam scenarios themselves**, scenario-first — per scenario: infographic, worked example, steppable living flow with mandate tags, worked code, exam-guide sample Q&A | Content-first (deliberately no SDK/agents — the sibling covers that); typed single-source data model, views as pure projections | **v0.3.0** — all 6 scenarios + all 12 exam-guide sample questions (v0.1); `/drill` recognition mode + reverse-link chips into the parent (v0.2); `/matrix` scenario×domain view, 12 anti-pattern foil pairs, print study sheets (v0.3) |
