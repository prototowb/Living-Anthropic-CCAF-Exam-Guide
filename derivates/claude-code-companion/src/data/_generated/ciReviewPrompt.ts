// CI review prompt body — Scenario 5 v0.4 task 9.
//
// Embeds the verbatim text of docs/CI_REVIEW_PROMPT.md so the in-browser
// PromptDissectionView can render it without a runtime fetch. The build is
// deterministic — the bundle ships the same prompt bytes the CI workflow
// loads from disk.
//
// This file lives under src/data/_generated/ for two reasons:
//   1. The path convention signals "do not hand-edit; regenerate from source."
//      v0.5 will add an `npm run sync:prompt` step that reads
//      docs/CI_REVIEW_PROMPT.md and re-writes this file, mirroring the
//      Scenario 6 extraction pipeline's pattern for derived content.
//   2. The browser bundle never reads from `docs/` directly — every file
//      consumed by the view layer must be importable as a TS module.
//
// Today the regeneration is manual. The string below is the byte-for-byte
// content of docs/CI_REVIEW_PROMPT.md (v1.0-2026-05-20). When the prompt is
// edited, also update PROMPT_VERSION below and re-paste the body.

/** The current CI prompt revision. Mirrors CURRENT_PROMPT_VERSION in
 *  src/agents/schemas/reviewOutput.ts. */
export const PROMPT_VERSION = 'v1.0-2026-05-20';

/** Verbatim contents of docs/CI_REVIEW_PROMPT.md. */
export const ciReviewPromptBody = `# CI review prompt — v1.0-2026-05-20
